...
const CpSetup = () => {
  ...
  const handleSubmit = async () => {
    ...
    const metadata = challengeData.map(
      ({ pseudoName, fullName, initialScore, timeLimit, memoryLimit }) => ({
        pseudoName,
        fullName,
        initialScore,
        timeLimit,
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_CE_SERVICE}/api/problems/upload-all`, {
        method: "POST",
        body: formData,
      });
      ...
      {infoVisible && (
        <div className={styles.alertBox}>
          ...
          <ul style={{ paddingLeft: "20px", marginBottom: "16px" }}>
            <li>
              <strong>Pseudo-name</strong> (must match the zip file name)
            </li>
            <li>
            ...
            <li>
              <strong>Time Limit to not exceed (s)</strong>
            </li>
            <li>
              <strong>Memory Limit to not exceed(MB)</strong>
            </li>
            <li>
